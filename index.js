...
function getPtByID(ID){
    ...
    let db = new sqlite3.Database('./rx.db', sqlite3.OPEN_READONLY, (err) => {
      if (err) {
        console.error(err.message);
      }
      console.log('Connected to the rx.db');
    });

    var sql = `SELECT DESYNPUF_ID as id,
    SRVC_DT as date,
    PROD_SRVC_ID as prodID,
    DAYS_SUPLY_NUM as daySupp
    FROM DE1_0_2008_to_2010_Prescription_Drug_Events_Sample_1
    WHERE DESYNPUF_ID = ?`
